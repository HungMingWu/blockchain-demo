...
namespace Checkpoints {
    ...
    double GuessVerificationProgress(const CCheckpointData& data, nonstd::observer_ptr<CBlockIndex> pindex, bool fSigchecks) {
        if (pindex==NULL)
            return 0.0;
    ...
    nonstd::observer_ptr<const CBlockIndex> GetLastCheckpoint(const CCheckpointData& data)
    {
        ...
        for (const MapCheckpoints::value_type& i : boost::adaptors::reverse(checkpoints))
        {
			...
			if (t != mapBlockIndex.end())
				return nonstd::make_observer(t->second.get());
        }
		return nullptr;
    }
