...
void RunTest(const TestVector &test) {
	...
	for (const TestDerivation &derive : test.vDerive) {
		...
		b58key.SetKey(key);
		REQUIRE(b58key.ToString() == derive.prv);
		...
		REQUIRE(b58pubkey.ToString() == derive.pub);
		...
		// Derive new keys
		CExtKey keyNew;
		REQUIRE(key.Derive(keyNew, derive.nChild));
		CExtPubKey pubkeyNew = keyNew.Neuter();
		if (!(derive.nChild & 0x80000000)) {
			// Compare with public derivation
			CExtPubKey pubkeyNew2;
			REQUIRE(pubkey.Derive(pubkeyNew2, derive.nChild));
			REQUIRE(pubkeyNew == pubkeyNew2);
		}
		key = keyNew;
		pubkey = pubkeyNew;
	}
