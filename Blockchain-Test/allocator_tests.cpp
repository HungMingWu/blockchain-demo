...
TEST_CASE("test_LockedPageManagerBase")
{
	...
	REQUIRE(last_lock_addr == reinterpret_cast<void*>(test_page_size*(600 + 500)));
	/* one very large object, page aligned */
	lpm.LockRange(reinterpret_cast<void*>(test_page_size * 1200), test_page_size * 500 - 1);
	REQUIRE(last_lock_addr == reinterpret_cast<void*>(test_page_size*(1200 + 500 - 1)));

	REQUIRE(lpm.GetLockedPageCount() == (
		(1000 * 33 + test_page_size - 1) / test_page_size + // small objects
		101 + 100 +  // page-sized objects
		501 + 500)); // large objects
	REQUIRE((last_lock_len & (test_page_size - 1)) == 0); // always lock entire pages
	REQUIRE(last_unlock_len == 0); // nothing unlocked yet
	...
	REQUIRE(lpm.GetLockedPageCount() == 0);
	...
	REQUIRE(lpm.GetLockedPageCount() == 0);
	REQUIRE((last_unlock_len & (test_page_size - 1)) == 0); // always unlock entire pages
}