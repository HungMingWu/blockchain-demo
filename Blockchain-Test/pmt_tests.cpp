...
TEST_CASE("pmt_test1")
{
	...
	for (int n = 0; n < 12; n++) {
		...
		uint256 merkleRoot1;
		std::tie(merkleRoot1, std::ignore) = BlockMerkleRoot(block);
		...
		for (int att = 1; att < 15; att++) {
			...
			REQUIRE(merkleRoot1 == merkleRoot2);
			REQUIRE(!merkleRoot2.IsNull());

			// check that it contains the matched transactions (in the same order!)
			REQUIRE(vMatchTxid1 == vMatchTxid2);
			...
			for (int j = 0; j < 4; j++) {
				...
				REQUIRE(merkleRoot3 != merkleRoot1);
			}
		}
	}
}

TEST_CASE("pmt_malleability")
{
	...
	REQUIRE(tree.ExtractMatches(vTxid).IsNull());
}