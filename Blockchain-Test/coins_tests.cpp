...
namespace
{
	...
	public:
		...
		void SelfTest() const
		{
			...
			REQUIRE(DynamicMemoryUsage() == ret);
		}
...
TEST_CASE("coins_cache_simulation_test")
{
	...
	for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
		...
		if (insecure_rand() % 1000 == 1 || i == NUM_SIMULATION_ITERATIONS - 1) {
			for (std::map<uint256, CCoins>::iterator it = result.begin(); it != result.end(); it++) {
				...
				if (coins) {
					REQUIRE(*coins == it->second);
					found_an_entry = true;
				}
				else {
					REQUIRE(it->second.IsPruned());
	...
	// Verify coverage.
	REQUIRE(removed_all_caches);
	REQUIRE(reached_4_caches);
	REQUIRE(added_an_entry);
	REQUIRE(removed_an_entry);
	REQUIRE(updated_an_entry);
	REQUIRE(found_an_entry);
	REQUIRE(missed_an_entry);
}
...
TEST_CASE("updatecoins_simulation_test")
{
	...
	// Verify coverage.
	REQUIRE(spent_a_duplicate_coinbase);
}