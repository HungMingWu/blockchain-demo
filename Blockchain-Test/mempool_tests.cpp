...
TEST_CASE("MempoolRemoveTest")
{
	...
	std::list<CTransaction> removed = testPool.remove(txParent, true);
	REQUIRE(removed.size() == 0);

	// Just the parent:
	testPool.addUnchecked(txParent.GetHash(), entry.FromTx(txParent));
	removed = testPool.remove(txParent, true);
	...
	removed = testPool.remove(txChild[0], true);
	REQUIRE(removed.size() == 2);
	removed.clear();
	// ... make sure grandchild and child are gone:
	removed = testPool.remove(txGrandChild[0], true);
	REQUIRE(removed.size() == 0);
	removed = testPool.remove(txChild[0], true);
	REQUIRE(removed.size() == 0);
	// Remove parent, all children/grandchildren should go:
	removed = testPool.remove(txParent, true);
	REQUIRE(removed.size() == 5);
	REQUIRE(testPool.size() == 0);
	removed.clear();
...
{
	REQUIRE(pool.size() == sortedOrder.size());
	...
	for (; it != pool.mapTx.get<index>().end(); ++it, ++count) {
		REQUIRE(it->GetTx().GetHash().ToString() == sortedOrder[count]);
	}
}
...
TEST_CASE("MempoolIndexingTest")
{
	...
	std::string dummy;
	REQUIRE(pool.CalculateMemPoolAncestors(entry.Fee(2000000LL).FromTx(tx7), setAncestorsCalculated, 100, 1000000, 1000, 1000000, dummy) == true);
	REQUIRE(setAncestorsCalculated == setAncestors);
	...
	REQUIRE(pool.CalculateMemPoolAncestors(entry.Fee(200000LL).Time(4).FromTx(tx10), setAncestorsCalculated, 100, 1000000, 1000, 1000000, dummy) == true);
	REQUIRE(setAncestorsCalculated == setAncestors);
	...
	std::list<CTransaction> removed = pool.remove(pool.mapTx.find(tx10.GetHash())->GetTx(), true);
	CheckSort<1>(pool, snapshotOrder);

	removed = pool.remove(pool.mapTx.find(tx9.GetHash())->GetTx(), true);
	removed = pool.remove(pool.mapTx.find(tx8.GetHash())->GetTx(), true);
...
TEST_CASE("MempoolSizeLimitTest")
{
	...
	REQUIRE(pool.exists(tx1.GetHash()));
	REQUIRE(pool.exists(tx2.GetHash()));
	...
	REQUIRE(!pool.exists(tx1.GetHash()));
	REQUIRE(pool.exists(tx2.GetHash()));
	REQUIRE(pool.exists(tx3.GetHash()));
	...
	REQUIRE(pool.GetMinFee(1).GetFeePerK() == maxFeeRateRemoved.GetFeePerK() + 1000);
	...
	REQUIRE(pool.exists(tx4.GetHash()));
	REQUIRE(pool.exists(tx6.GetHash()));
	REQUIRE(!pool.exists(tx7.GetHash()));
	...
	REQUIRE(pool.exists(tx4.GetHash()));
	REQUIRE(!pool.exists(tx5.GetHash()));
	REQUIRE(pool.exists(tx6.GetHash()));
	REQUIRE(!pool.exists(tx7.GetHash()));
	...
	conflicts = pool.removeForBlock(vtx, 1);
	SetMockTime(42 + 2 * CTxMemPool::ROLLING_FEE_HALFLIFE);
	REQUIRE(pool.GetMinFee(1).GetFeePerK() == (maxFeeRateRemoved.GetFeePerK() + 1000) / 2);
	// ... then feerate should drop 1/2 each halflife

	SetMockTime(42 + 2 * CTxMemPool::ROLLING_FEE_HALFLIFE + CTxMemPool::ROLLING_FEE_HALFLIFE / 2);
	REQUIRE(pool.GetMinFee(pool.DynamicMemoryUsage() * 5 / 2).GetFeePerK() == (maxFeeRateRemoved.GetFeePerK() + 1000) / 4);
	// ... with a 1/2 halflife when mempool is < 1/2 its target size

	SetMockTime(42 + 2 * CTxMemPool::ROLLING_FEE_HALFLIFE + CTxMemPool::ROLLING_FEE_HALFLIFE / 2 + CTxMemPool::ROLLING_FEE_HALFLIFE / 4);
	REQUIRE(pool.GetMinFee(pool.DynamicMemoryUsage() * 9 / 2).GetFeePerK() == (maxFeeRateRemoved.GetFeePerK() + 1000) / 8);
	// ... with a 1/4 halflife when mempool is < 1/4 its target size

	SetMockTime(42 + 7 * CTxMemPool::ROLLING_FEE_HALFLIFE + CTxMemPool::ROLLING_FEE_HALFLIFE / 2 + CTxMemPool::ROLLING_FEE_HALFLIFE / 4);
	REQUIRE(pool.GetMinFee(1).GetFeePerK() == 1000);
	// ... but feerate should never drop below 1000

	SetMockTime(42 + 8 * CTxMemPool::ROLLING_FEE_HALFLIFE + CTxMemPool::ROLLING_FEE_HALFLIFE / 2 + CTxMemPool::ROLLING_FEE_HALFLIFE / 4);
	REQUIRE(pool.GetMinFee(1).GetFeePerK() == 0);
