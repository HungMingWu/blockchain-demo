...
{
	REQUIRE(CNetAddr("127.0.0.1").GetGroup() == std::vector<unsigned char>{0x01}); // Local -> !Routable()
	REQUIRE(CNetAddr("257.0.0.1").GetGroup() == std::vector<unsigned char>{0x02, 0x00, 0x00, 0x00, 0x00}); // !Valid -> !Routable()
	REQUIRE(CNetAddr("10.0.0.1").GetGroup() == std::vector<unsigned char>{0x01, 0x0a, 0x00}); // RFC1918 -> !Routable()
	REQUIRE(CNetAddr("169.254.1.1").GetGroup() == std::vector<unsigned char>{0x01, 0xa9, 0xfe}); // RFC3927 -> !Routable()
	REQUIRE(CNetAddr("1.2.3.4").GetGroup() == std::vector<unsigned char>{NET_IPV4,1,2}); // IPv4
	REQUIRE(CNetAddr("::FFFF:0:102:304").GetGroup() == std::vector<unsigned char>{NET_IPV4, 1, 2}); // RFC6145
	REQUIRE(CNetAddr("64:FF9B::102:304").GetGroup() == std::vector<unsigned char>{NET_IPV4, 1, 2}); // RFC6052
	REQUIRE(CNetAddr("2002:102:304:9999:9999:9999:9999:9999").GetGroup() == std::vector<unsigned char>{NET_IPV4, 1, 2}); // RFC3964
	REQUIRE(CNetAddr("2001:0:9999:9999:9999:9999:FEFD:FCFB").GetGroup() == std::vector<unsigned char>{NET_IPV4, 1, 2}); // RFC4380
	REQUIRE(CNetAddr("FD87:D87E:EB43:edb1:8e4:3588:e546:35ca").GetGroup() == std::vector<unsigned char>{NET_TOR, 239}); // Tor
	REQUIRE(CNetAddr("2001:470:abcd:9999:9999:9999:9999:9999").GetGroup() == std::vector<unsigned char>{NET_IPV6, 32, 1, 4, 112, 175}); //he.net
	REQUIRE(CNetAddr("2001:2001:9999:9999:9999:9999:9999:9999").GetGroup() == std::vector<unsigned char>{NET_IPV6, 32, 1, 32, 1}); //IPv6
}