...
TEST_CASE_METHOD(BasicTestingSetup, "bloom_create_insert_serialize")
{
	CBloomFilter filter(3, 0.01, 0, BLOOM_UPDATE_ALL);

	filter.insert(ParseHex("99108ad8ed9bb6274d3980bab5a85c048f0950c8"));
	REQUIRE_MESSAGE(filter.contains(ParseHex("99108ad8ed9bb6274d3980bab5a85c048f0950c8")), "BloomFilter doesn't contain just-inserted object!");

	// One bit different in first byte
	REQUIRE_MESSAGE(!filter.contains(ParseHex("19108ad8ed9bb6274d3980bab5a85c048f0950c8")), "BloomFilter contains something it shouldn't!");

	filter.insert(ParseHex("b5a2c786d9ef4658287ced5914b37a1b4aa32eee"));
	REQUIRE_MESSAGE(filter.contains(ParseHex("b5a2c786d9ef4658287ced5914b37a1b4aa32eee")), "BloomFilter doesn't contain just-inserted object (2)!");

	filter.insert(ParseHex("b9300670b4c5366e95b2699e8b18bc75e5f729c5"));
	REQUIRE_MESSAGE(filter.contains(ParseHex("b9300670b4c5366e95b2699e8b18bc75e5f729c5")), "BloomFilter doesn't contain just-inserted object (3)!");
	...
	vector<char> streamVec(stream.begin(), stream.end());
	for (unsigned int i = 0; i < vch.size(); i++)
		expected[i] = (char)vch[i];

	REQUIRE(streamVec == expected);

	REQUIRE_MESSAGE(filter.contains(ParseHex("99108ad8ed9bb6274d3980bab5a85c048f0950c8")), "BloomFilter doesn't contain just-inserted object!");
	filter.clear();
	REQUIRE_MESSAGE(!filter.contains(ParseHex("99108ad8ed9bb6274d3980bab5a85c048f0950c8")), "BloomFilter should be empty!");
}
...
TEST_CASE_METHOD(BasicTestingSetup, "bloom_create_insert_key")
{
	string strSecret = string("cNU9ZAikd1fjVM5ALpsbq7zjn4BFcT9LebkpYR8Am8RJ7FAr1SeZ");
	CBitcoinSecret vchSecret;
	REQUIRE(vchSecret.SetString(strSecret));
...
TEST_CASE("bloom_match")
{
	...
	REQUIRE_MESSAGE(filter.IsRelevantAndUpdate(tx), "Simple Bloom filter didn't match tx hash");

	filter = CBloomFilter(10, 0.000001, 0, BLOOM_UPDATE_ALL);
	// byte-reversed tx hash
	filter.insert(ParseHex("6bff7fcd4f8565ef406dd5d63d4ff94f318fe82027fd4dc451b04474019f74b4"));
	REQUIRE_MESSAGE(filter.IsRelevantAndUpdate(tx), "Simple Bloom filter didn't match manually serialized tx hash");
	...
	REQUIRE_MESSAGE(filter.IsRelevantAndUpdate(tx), "Simple Bloom filter didn't match input signature");
	...
	REQUIRE_MESSAGE(filter.IsRelevantAndUpdate(tx), "Simple Bloom filter didn't match input pub key");

	filter = CBloomFilter(10, 0.000001, 0, BLOOM_UPDATE_ALL);
	filter.insert(ParseHex("04943fdd508053c75000106d3bc6e2754dbcff19"));
	REQUIRE_MESSAGE(filter.IsRelevantAndUpdate(tx), "Simple Bloom filter didn't match output address");
	REQUIRE_MESSAGE(filter.IsRelevantAndUpdate(spendingTx), "Simple Bloom filter didn't add output");
	...
	REQUIRE_MESSAGE(filter.IsRelevantAndUpdate(tx), "Simple Bloom filter didn't match COutPoint");
	...
	REQUIRE_MESSAGE(filter.IsRelevantAndUpdate(tx), "Simple Bloom filter didn't match manually serialized COutPoint");

	filter = CBloomFilter(10, 0.000001, 0, BLOOM_UPDATE_ALL);
	filter.insert(uint256S("00000009e784f32f62ef849763d4f45b98e07ba658647343b915ff832b110436"));
	REQUIRE_MESSAGE(!filter.IsRelevantAndUpdate(tx), "Simple Bloom filter matched random tx hash");

	filter = CBloomFilter(10, 0.000001, 0, BLOOM_UPDATE_ALL);
	filter.insert(ParseHex("0000006d2965547608b9e15d9032a7b9d64fa431"));
	REQUIRE_MESSAGE(!filter.IsRelevantAndUpdate(tx), "Simple Bloom filter matched random address");

	filter = CBloomFilter(10, 0.000001, 0, BLOOM_UPDATE_ALL);
	filter.insert(COutPoint(uint256S("0x90c122d70786e899529d71dbeba91ba216982fb6ba58f3bdaab65e73b7e9260b"), 1));
	REQUIRE_MESSAGE(!filter.IsRelevantAndUpdate(tx), "Simple Bloom filter matched COutPoint for an output we didn't care about");
...
TEST_CASE("merkle_block_1")
{
	...
	REQUIRE(merkleBlock.header.GetHash() == block.GetHash());

	REQUIRE(merkleBlock.vMatchedTxn.size() == 1);
	pair<unsigned int, uint256> pair = merkleBlock.vMatchedTxn[0];

	REQUIRE(merkleBlock.vMatchedTxn[0].second == uint256S("0x2b20079614e39eeffc856e2c06ed6abb7dc5bd4e3ea2235ae494b5efe92dff30"));
	REQUIRE(merkleBlock.vMatchedTxn[0].first == 2);

	vector<uint256> vMatched;
	REQUIRE(merkleBlock.txn.ExtractMatches(vMatched) == block.hashMerkleRoot);
	REQUIRE(vMatched.size() == merkleBlock.vMatchedTxn.size());
	for (unsigned int i = 0; i < vMatched.size(); i++)
		REQUIRE(vMatched[i] == merkleBlock.vMatchedTxn[i].second);
	...
	REQUIRE(merkleBlock.vMatchedTxn.size() == 2);

	REQUIRE(merkleBlock.vMatchedTxn[1] == pair);
	REQUIRE(merkleBlock.vMatchedTxn[0].second == uint256S("0x9286f154605d2380b0e0f38dd3a0d32eb4755631315b29110691e8bdeaa870e4"));

	REQUIRE(merkleBlock.vMatchedTxn[0].first == 0);
...
TEST_CASE("merkle_block_2")
{
	...
	REQUIRE(merkleBlock.vMatchedTxn[0].second == uint256S("0x5211f6c82d2d3687afe6b6ae90424fe8199f7f1fe1c3c2d70275ea28638b2c1b"));
	REQUIRE(merkleBlock.vMatchedTxn[0].first == 0);
	...
	REQUIRE(pair == merkleBlock.vMatchedTxn[0]);
...
TEST_CASE("merkle_block_2_with_update_none")
{
	...
	REQUIRE(merkleBlock.vMatchedTxn[0].second == uint256S("0x1f5ac80b3d26c50ffd143d98adfd53d529aca80d4dbd18c8c40d5a7d829234aa"));
	REQUIRE(merkleBlock.vMatchedTxn[0].first == 0);
...
TEST_CASE("merkle_block_3_and_serialize")
{
	...
	REQUIRE(merkleBlock.vMatchedTxn[0].second == uint256S("0x16e02788565e50d059a75e254b622a099527ecdf29ad2498454b3c30727d0272"));
	REQUIRE(merkleBlock.vMatchedTxn[0].first == 0);
	...
	vector<char> expected(vch.size());
	vector<char> merkleStreamVec(merkleStream.begin(), merkleStream.end());
	for (unsigned int i = 0; i < vch.size(); i++)
		expected[i] = (char)vch[i];

	REQUIRE(expected == merkleStreamVec);
}
...
TEST_CASE("merkle_block_4")
{
	...
	REQUIRE(merkleBlock.vMatchedTxn[0].second == uint256S("0x43b8ced49f4626174806456cf79e4bc2465249bfc5a4a83d83bd67e63eecee3b"));
	REQUIRE(merkleBlock.vMatchedTxn[0].first == 3);
	...
	REQUIRE(merkleBlock.vMatchedTxn[0].second == uint256S("0x76fc7a9690a04ec4c9f3233cd8a45abf5bcebeba76491c5502f3f81eb8d753b0"));
	REQUIRE(merkleBlock.vMatchedTxn[0].first == 0);
...
TEST_CASE("merkle_block_4_test_p2pubkey_only")
{
	...
	REQUIRE(filter.contains(COutPoint(uint256S("0x37da235d0566c91b7fd5f41e7861b63ee8c6ff3ab0f37a39ab128fa5aafc25f8"), 0)));
	// ... but not the 4th transaction's output (its not pay-2-pubkey)
	REQUIRE(!filter.contains(COutPoint(uint256S("0x02981fa052f0481dbc5868f4fc2166035a10f27a03cfd2de67326471df5bc011"), 0)));
}
...
TEST_CASE("merkle_block_4_test_update_none")
{
	...
	REQUIRE(!filter.contains(COutPoint(uint256S("0x37da235d0566c91b7fd5f41e7861b63ee8c6ff3ab0f37a39ab128fa5aafc25f8"), 0)));
	REQUIRE(!filter.contains(COutPoint(uint256S("0x02981fa052f0481dbc5868f4fc2166035a10f27a03cfd2de67326471df5bc041"), 0)));
}
...
TEST_CASE("rolling_bloom")
{
	...
	INFO("RollingBloomFilter got " << nHits << " false positives (~100 expected)");

	// Insanely unlikely to get a fp count outside this range:
	REQUIRE(nHits > 25);
	REQUIRE(nHits < 175);

	REQUIRE(rb1.contains(data[DATASIZE - 1]));
	rb1.reset();
	REQUIRE(!rb1.contains(data[DATASIZE - 1]));
	...
	for (int i = 0; i < DATASIZE; i++) {
		if (i >= 100)
			REQUIRE(rb1.contains(data[i - 100]));
		rb1.insert(data[i]);
	}
	...
	INFO("RollingBloomFilter got " << nHits << " false positives (~5 expected)");
	REQUIRE(nHits < 100);
