...
//is a claim in queue 
bool is_claim_in_queue(std::string name, const CTransaction &tx)
{
	...
	if (have_claim) {
		...
		INFO("Is not a claim in queue.");
		return false;
	}
}

// check if tx is best claim based on outpoint 
bool is_best_claim(std::string name, const CTransaction &tx)
{
	CClaimValue val;
	COutPoint outPoint(tx.GetHash(), 0);
	...
	if (have_claim && have_info && val.outPoint == outPoint) {
		...
		INFO("Is not best claim");
		return false;
	}
}
// check effective quantity of best claim
bool best_claim_effective_amount_equals(std::string name, CAmount amount)
{
	CClaimValue val;
	bool have_info = pclaimTrie->getInfoForName(name, val);
	if (!have_info)
	...
	else
	{
		...
		{
			std::string str = std::to_string(amount) + " != " + std::to_string(effective_amount);
			INFO(str);
			return false;
		}
		else
...
{
	auto pblocktemplate = CreateNewBlock(Params(), CScript() << OP_TRUE);
	coinbases.clear();
	REQUIRE(pblocktemplate);
	...
	{
		REQUIRE(CreateBlock(pblocktemplate.get()));
...
struct ClaimTrieChainFixture {
	...
	void CommitTx(CMutableTransaction &tx) {
		...
		CValidationState state = AcceptToMemoryPool(mempool, tx, false, &fMissingInputs, &txFeeRate);
	...
	void IncrementBlocks(int num_blocks, bool mark = false)
	{
		...
		for (int i = 0; i < num_blocks; ++i)
		{
			...
			auto pblocktemplate = CreateNewBlock(Params(), coinbase_scriptpubkey);
			REQUIRE(pblocktemplate);
			REQUIRE(pblocktemplate->block.vtx.size() == num_txs_for_next_block + 1);
			pblocktemplate->block.hashPrevBlock = chainActive.Tip()->GetBlockHash();
			REQUIRE(CreateBlock(pblocktemplate.get()));
...
TEST_CASE_METHOD(RegTestingSetup, "claim_test")
{
	...
	fixture.IncrementBlocks(1);
	REQUIRE(is_best_claim("test", tx3));
	REQUIRE(2U == pclaimTrie->getClaimsForName("test").claims.size());

	fixture.DecrementBlocks(1);
	REQUIRE(!is_best_claim("test", tx2));
	REQUIRE(!is_best_claim("test", tx3));
	REQUIRE(0U == pclaimTrie->getClaimsForName("test").claims.size());

	// make two claims , one older 
	CMutableTransaction tx4 = fixture.MakeClaim(fixture.GetCoinbase(), "test", "one", 1);
	fixture.IncrementBlocks(1);
	REQUIRE(is_best_claim("test", tx4));
	CMutableTransaction tx5 = fixture.MakeClaim(fixture.GetCoinbase(), "test", "two", 1);
	fixture.IncrementBlocks(1);
	REQUIRE(is_claim_in_queue("test", tx5));
	REQUIRE(is_best_claim("test", tx4));
	fixture.IncrementBlocks(1);
	REQUIRE(is_best_claim("test", tx4));
	REQUIRE(2U == pclaimTrie->getClaimsForName("test").claims.size());
	...
	REQUIRE(is_best_claim("test", tx6));
	CMutableTransaction tx7 = fixture.MakeClaim(fixture.GetCoinbase(), "test", "two", 2);
	fixture.IncrementBlocks(1);
	REQUIRE(is_claim_in_queue("test", tx7));
	REQUIRE(is_best_claim("test", tx6));
	fixture.IncrementBlocks(10);
	REQUIRE(is_best_claim("test", tx7));
	REQUIRE(2U == pclaimTrie->getClaimsForName("test").claims.size());
...
TEST_CASE("spend_claim_test")
{
	...
	fixture.IncrementBlocks(1);
	REQUIRE(is_best_claim("test", tx5));
	fixture.Spend(tx5);
	fixture.IncrementBlocks(1);
	REQUIRE(!is_best_claim("test", tx5));
...
TEST_CASE("support_test")
{
	...
	fixture.IncrementBlocks(1);
	REQUIRE(is_best_claim("test", tx2));
	REQUIRE(best_claim_effective_amount_equals("test", 11));
	fixture.DecrementBlocks(1);
	...
	fixture.IncrementBlocks(10);
	REQUIRE(is_best_claim("test", tx4));
	REQUIRE(best_claim_effective_amount_equals("test", 2));
...
*/
TEST_CASE("support_on_abandon_test")
{
	ClaimTrieChainFixture fixture;
	CMutableTransaction tx1 = fixture.MakeClaim(fixture.GetCoinbase(), "test", "one", 2);
	fixture.IncrementBlocks(1);
...
TEST_CASE("support_on_abandon_2_test")
{
	ClaimTrieChainFixture fixture;
	CMutableTransaction tx1 = fixture.MakeClaim(fixture.GetCoinbase(), "test", "one", 2);
	CMutableTransaction s1 = fixture.MakeSupport(fixture.GetCoinbase(), tx1, "test", 1);
...
TEST_CASE_METHOD(RegTestingSetup, "update_on_support_test")
{
...
*/
TEST_CASE("claimtrie_update_test")
{
	//update preserves claim id 
	ClaimTrieChainFixture fixture;
	CMutableTransaction tx1 = fixture.MakeClaim(fixture.GetCoinbase(), "test", "one", 2);
	...
	REQUIRE(val.claimId == ClaimIdHash(tx1.GetHash(), 0));
	REQUIRE(is_best_claim("test", u1));
	fixture.DecrementBlocks(1);
...
*/
TEST_CASE("claimtrie_expire_test")
{
	ClaimTrieChainFixture fixture;
 ...
 */
TEST_CASE("claimtriebranching_get_effective_amount_for_claim")
{
	...
	REQUIRE(pclaimTrie->getEffectiveAmountForClaim("test", claimId) == 2);
	REQUIRE(pclaimTrie->getEffectiveAmountForClaim("inexistent", claimId) == 0); //not found returns 0

	// one claim, one support
	fixture.MakeSupport(fixture.GetCoinbase(), claimtx, "test", 40);
	fixture.IncrementBlocks(1);

	REQUIRE(pclaimTrie->getEffectiveAmountForClaim("test", claimId) == 42);
	...
	REQUIRE(pclaimTrie->getEffectiveAmountForClaim("test", claimId) == 42);
	REQUIRE(pclaimTrie->getEffectiveAmountForClaim("test", claimId2) == 1);
	REQUIRE(pclaimTrie->getEffectiveAmountForClaim("inexistent", claimId) == 0);
	REQUIRE(pclaimTrie->getEffectiveAmountForClaim("inexistent", claimId2) == 0);
	...
	REQUIRE(pclaimTrie->getEffectiveAmountForClaim("test", claimId) == 42);
	REQUIRE(pclaimTrie->getEffectiveAmountForClaim("test", claimId2) == 7);
	...
	REQUIRE(pclaimTrie->getEffectiveAmountForClaim("test", claimId) == 42);
	REQUIRE(pclaimTrie->getEffectiveAmountForClaim("test", claimId2) == 1337);
}

/*
 * tests for CClaimTrie::getClaimById basic consistency checks
 */
TEST_CASE("get_claim_by_id_test")
{
	ClaimTrieChainFixture fixture;
	...
	REQUIRE(claimName == name);
	REQUIRE(claimValue.claimId == claimId);
	...
	REQUIRE(claimName == name);
	REQUIRE(claimValue.claimId == claimId2);
	...
	REQUIRE(claimName == name);
	REQUIRE(claimValue.claimId == claimId);
	REQUIRE(claimValue.nAmount == 1);
	REQUIRE(claimValue.outPoint.hash == u1.GetHash());

	fixture.Spend(u1);
	fixture.IncrementBlocks(1);
	REQUIRE(!pclaimTrie->getClaimById(claimId, claimName, claimValue));
	...
	REQUIRE(claimName != name);
	REQUIRE(claimValue2.claimId != claimId2);
	...
	REQUIRE(claimName != name);
	REQUIRE(claimValue2.claimId != claimId);
}
...
*/
TEST_CASE("hardfork_claim_test")
{
	ClaimTrieChainFixture fixture;

	REQUIRE(pclaimTrie->nExpirationTime == fixture.originalExpiration);
	...
	fixture.IncrementBlocks(1);
	REQUIRE(chainActive.Height() == fixture.expirationForkHeight - 1);

	// Disable future expirations and fast-forward past the fork height
	fixture.IncrementBlocks(1);
	REQUIRE(chainActive.Height() == fixture.expirationForkHeight);
	REQUIRE(pclaimTrie->nExpirationTime == fixture.extendedExpiration);
	...
	REQUIRE(is_best_claim("test2", tx2));
...
*/
TEST_CASE("hardfork_support_test")
{
	ClaimTrieChainFixture fixture;
	...
	REQUIRE(is_best_claim("test", tx1));
	REQUIRE(best_claim_effective_amount_equals("test", 3));
	...
	fixture.IncrementBlocks(1);
	REQUIRE(fixture.expirationForkHeight == chainActive.Height());

	REQUIRE(is_best_claim("test", u1));
	REQUIRE(best_claim_effective_amount_equals("test", 3));
	REQUIRE(!is_claim_in_queue("test", tx1));
	REQUIRE(!is_claim_in_queue("test", tx2));
...
TEST_CASE("hardfork_disk_test")
{
	...
	// increment to fork
	fixture.IncrementBlocks(fixture.expirationForkHeight - chainActive.Height());
	REQUIRE(is_best_claim("test2", u2));
	REQUIRE(best_claim_effective_amount_equals("test2", 2));
	...
	REQUIRE(is_best_claim("test2", u2));
	REQUIRE(best_claim_effective_amount_equals("test2", 1)); // the support expires one block before
}

TEST_CASE("insert_update_claim_test")
{
	ClaimTrieChainFixture fixture;
...
TEST_CASE("basic_merkle_test")
{
	...
	REQUIRE(tx1MerkleHash != pclaimTrie->getMerkleHash());
	fixture.CommitTx(tx1);
	fixture.IncrementBlocks(20);
	REQUIRE(tx1MerkleHash == pclaimTrie->getMerkleHash());
}

TEST_CASE("claim_expiration_test")
{
	ClaimTrieChainFixture fixture;
	...
	REQUIRE(!pclaimTrie->empty());
	REQUIRE(pclaimTrie->queueEmpty());
	REQUIRE(!pclaimTrie->expirationQueueEmpty());
	...
	REQUIRE(!pclaimTrie->queueEmpty());
	REQUIRE(pclaimTrie->haveClaimInQueue(sName, tx3OutPoint, nThrowaway));
	...
	REQUIRE(pclaimTrie->queueEmpty());
	REQUIRE(!pclaimTrie->expirationQueueEmpty());
	REQUIRE(pclaimTrie->getInfoForName(sName, val));
	REQUIRE(val.outPoint == tx1OutPoint);
	uint256 tx1MerkleHash = pclaimTrie->getMerkleHash();
...
TEST_CASE("claimtrienode_serialize_unserialize")
{
...
TEST_CASE("invalid_claimid_test")
{
	ClaimTrieChainFixture fixture;
	...
	REQUIRE(pcoinsTip->HaveCoins(tx1.GetHash()));
	REQUIRE(!pclaimTrie->empty());
	REQUIRE(pclaimTrie->queueEmpty());
	REQUIRE(pclaimTrie->supportEmpty());
	REQUIRE(pclaimTrie->supportQueueEmpty());
	...
	REQUIRE(pcoinsTip->HaveCoins(tx2.GetHash()));
	REQUIRE(!pclaimTrie->empty());
	REQUIRE(!pclaimTrie->queueEmpty());
	...
	REQUIRE(pcoinsTip->HaveCoins(tx3.GetHash()));
	REQUIRE(!pclaimTrie->empty());
	REQUIRE(pclaimTrie->queueEmpty());

	// Verify it's not in the claim trie
	REQUIRE(pclaimTrie->getInfoForName(sName, val));
	REQUIRE(val.outPoint.hash == tx1.GetHash());

	REQUIRE(!pclaimTrie->haveClaim(sName, tx4OutPoint));

	// Update the tx with the bogus claimId
	fixture.CommitTx(tx4);
	fixture.IncrementBlocks(1); // 104

	REQUIRE(pcoinsTip->HaveCoins(tx4.GetHash()));
	REQUIRE(!pclaimTrie->empty());
	REQUIRE(pclaimTrie->queueEmpty());
...
TEST_CASE("supporting_claims_test")
{
	ClaimTrieChainFixture fixture;
	...
	REQUIRE(pclaimTrie->supportQueueEmpty());
	REQUIRE(pclaimTrie->getInfoForName(sName, val));
	REQUIRE(val.outPoint.hash == tx2.GetHash());
	REQUIRE(tx1MerkleHash != pclaimTrie->getMerkleHash());
	...
	REQUIRE(pcoinsTip->HaveCoins(tx7.GetHash()));
	REQUIRE(!pclaimTrie->empty());
	REQUIRE(pclaimTrie->queueEmpty());
	REQUIRE(!pclaimTrie->supportEmpty());
	REQUIRE(pclaimTrie->supportQueueEmpty());
	REQUIRE(pclaimTrie->getInfoForName(sName, val));
	REQUIRE(val.outPoint.hash == tx7.GetHash());
	REQUIRE(tx1MerkleHash != pclaimTrie->getMerkleHash());
	...
	REQUIRE(initialHeight == chainActive.Height());
...
TEST_CASE("supporting_claims2_test")
{
	ClaimTrieChainFixture fixture;
	...
	REQUIRE(pclaimTrie->haveClaimInQueue(sName, tx2cp, nThrowaway));
	...
	REQUIRE(pclaimTrie->getInfoForName(sName, val));
	REQUIRE(val.outPoint.hash == tx1.GetHash());
	REQUIRE(rootMerkleHash == pclaimTrie->getMerkleHash());
	...
	REQUIRE(val.outPoint.hash == tx1.GetHash());
	REQUIRE(rootMerkleHash == pclaimTrie->getMerkleHash());
	REQUIRE(pclaimTrie->haveClaim(sName, tx2cp));
	...
	REQUIRE(pclaimTrie->haveClaim(sName, tx1cp));
...
TEST_CASE("expiring_supports_test")
{
	ClaimTrieChainFixture fixture;
	...
	REQUIRE(pclaimTrie->supportQueueEmpty());
	REQUIRE(pclaimTrie->getInfoForName(sName, val));
	REQUIRE(val.outPoint.hash == tx4.GetHash());
	REQUIRE(rootMerkleHash != pclaimTrie->getMerkleHash());
...
TEST_CASE("value_proof_test")
{
	...
	REQUIRE(val.outPoint == tx1OutPoint);
	REQUIRE(pclaimTrie->getInfoForName(sName2, val));
	REQUIRE(val.outPoint == tx2OutPoint);
	REQUIRE(pclaimTrie->getInfoForName(sName3, val));
	REQUIRE(val.outPoint == tx3OutPoint);
	REQUIRE(pclaimTrie->getInfoForName(sName4, val));
	REQUIRE(val.outPoint == tx4OutPoint);
	...
	REQUIRE(verify_proof(proof, chainActive.Tip()->hashClaimTrie, sName1));
	REQUIRE(proof.outPoint == tx1OutPoint);

	proof = cache.getProofForName(sName2);
	REQUIRE(verify_proof(proof, chainActive.Tip()->hashClaimTrie, sName2));
	REQUIRE(proof.outPoint == tx2OutPoint);

	proof = cache.getProofForName(sName3);
	REQUIRE(verify_proof(proof, chainActive.Tip()->hashClaimTrie, sName3));
	REQUIRE(proof.outPoint == tx3OutPoint);

	proof = cache.getProofForName(sName4);
	REQUIRE(verify_proof(proof, chainActive.Tip()->hashClaimTrie, sName4));
	REQUIRE(proof.outPoint == tx4OutPoint);

	proof = cache.getProofForName(sName5);
	REQUIRE(verify_proof(proof, chainActive.Tip()->hashClaimTrie, sName5));
	REQUIRE(proof.hasValue == false);

	proof = cache.getProofForName(sName6);
	REQUIRE(verify_proof(proof, chainActive.Tip()->hashClaimTrie, sName6));
	REQUIRE(proof.hasValue == false);

	proof = cache.getProofForName(sName7);
	REQUIRE(verify_proof(proof, chainActive.Tip()->hashClaimTrie, sName7));
	REQUIRE(proof.hasValue == false);
	...
	REQUIRE(pclaimTrie->getInfoForName(sName7, val));
	REQUIRE(val.outPoint == tx5OutPoint);

	CClaimTrieCache cache1(*pclaimTrie);

	proof = cache1.getProofForName(sName1);
	REQUIRE(verify_proof(proof, chainActive.Tip()->hashClaimTrie, sName1));
	REQUIRE(proof.outPoint == tx1OutPoint);

	proof = cache1.getProofForName(sName2);
	REQUIRE(verify_proof(proof, chainActive.Tip()->hashClaimTrie, sName2));
	REQUIRE(proof.outPoint == tx2OutPoint);

	proof = cache1.getProofForName(sName3);
	REQUIRE(verify_proof(proof, chainActive.Tip()->hashClaimTrie, sName3));
	REQUIRE(proof.outPoint == tx3OutPoint);

	proof = cache1.getProofForName(sName4);
	REQUIRE(verify_proof(proof, chainActive.Tip()->hashClaimTrie, sName4));
	REQUIRE(proof.outPoint == tx4OutPoint);

	proof = cache1.getProofForName(sName5);
	REQUIRE(verify_proof(proof, chainActive.Tip()->hashClaimTrie, sName5));
	REQUIRE(proof.hasValue == false);

	proof = cache1.getProofForName(sName6);
	REQUIRE(verify_proof(proof, chainActive.Tip()->hashClaimTrie, sName6));
	REQUIRE(proof.hasValue == false);

	proof = cache1.getProofForName(sName7);
	REQUIRE(verify_proof(proof, chainActive.Tip()->hashClaimTrie, sName7));
	REQUIRE(proof.outPoint == tx5OutPoint);
...
TEST_CASE("bogus_claimtrie_hash_test")
{
	ClaimTrieChainFixture fixture;
	...
	auto pblockTemp = CreateNewBlock(Params(), tx1.vout[0].scriptPubKey);
	REQUIRE(pblockTemp);
	...
	std::tie(pblockTemp->block.hashMerkleRoot, std::ignore) = BlockMerkleRoot(pblockTemp->block);
	//create bogus hash
	...
	for (arith_uint256 i = 0;; ++i) {
		pblockTemp->block.nNonce = ArithToUint256(i);
		if (CheckProofOfWork(pblockTemp->block.GetHash(), pblockTemp->block.nBits, Params().GetConsensus())) {
			break;
		}
	}
	CValidationState state = ProcessNewBlock(Params(), NULL, &pblockTemp->block, true, boost::none);
	bool success = state.IsValid();
	// will process , but will not be connected
	REQUIRE(success);
	REQUIRE(state.IsValid());
	REQUIRE(pblockTemp->block.GetHash() != chainActive.Tip()->GetBlockHash());
	REQUIRE(orig_chain_height == chainActive.Height());
}

/*
 * tests for CClaimTrie::getClaimById basic consistency checks
 */
TEST_CASE("get_claim_by_id_test_2")
{
	ClaimTrieChainFixture fixture;
	const std::string name = "test";
	...
	fixture.IncrementBlocks(1);
	REQUIRE(!pclaimTrie->getClaimById(claimId, claimName, claimValue));
	REQUIRE(!pclaimTrie->getClaimById(claimIdx, claimName, claimValue));
	...
	REQUIRE(claimName == name);
	REQUIRE(claimValue.claimId == claimIdx);
}

TEST_CASE("get_claim_by_id_test_3")
{
	ClaimTrieChainFixture fixture;
	pclaimTrie->setExpirationTime(5);
	const std::string name = "test";
	...
	REQUIRE(is_best_claim(name, tx1));
	REQUIRE(pclaimTrie->getClaimById(claimId2, claimName, claimValue));
	REQUIRE(claimName == name);
	REQUIRE(claimValue.claimId == claimId2);

	fixture.IncrementBlocks(1);
	// second claim has activated
	REQUIRE(is_best_claim(name, tx2));
	REQUIRE(pclaimTrie->getClaimById(claimId2, claimName, claimValue));
	REQUIRE(claimName == name);
	REQUIRE(claimValue.claimId == claimId2);
